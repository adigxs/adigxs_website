...
if($show_user_review_form): ?>
    ...
    <form action="<?php echo esc_url(get_permalink($post->ID)); ?>"
        ...
        <div class="xs-review-box public-xs-review-box" id="xs-review-box">
			...
			<?php
			if(isset($_COOKIE['xs_review_message']) && strlen(sanitize_text_field(wp_unslash($_COOKIE['xs_review_message']))) > 4 && isset($_POST['xs_review_form_public_data']) ) { //phpcs:ignore  ?>
                <div class="review_message_show">
                    <p><?php echo esc_html(sanitize_text_field(wp_unslash($_COOKIE['xs_review_message'])), 'wp-ultimate-review'); //phpcs:ignore (WordPress.Security.ValidatedSanitizedInput.MissingUnslash
